class Stop < ActiveRecord::Base
  ...
  scope :mis_n, ->(mis_name) { where( mi_system_id: MiSystem.where( name: mis_name ).map(&:id) ) unless mis_name.blank? }
  ...
  scope :neighbours_n, ->(neighbour_stop_code, dist) { where ( "ST_DWithin(shape, '#{ Stop.where(stop_code: neighbour_stop_code).shape.as_text}', #{dist})") unless neighbour_stop_code.blank? }
  ...
  scope :has_transtions, ->(has_transition) { where( id: (Connection.all.map(&:stop_1_id) + Connection.all.map(&:stop_2_id)).uniq ) unless (has_transition.to_i == 0) || (Connection.all.map(&:stop_1_id).empty? && Connection.all.map(&:stop_2_id).empty?) }

  scope :searching, ->(args){mis(args[:mis_id]).mis_n(args[:mis_name]).identifier(args[:stop_id]).named_like(args[:stop_name]).admin_code(args[:administrative_code]).postal_code(args[:postal_code]).st_code(args[:stop_code]).neighbours(args[:neighbour_stop_id], args[:dist]).neighbours_n(args[:neighbour_stop_code], args[:dist]).has_transtions(args[:has_transition])}
